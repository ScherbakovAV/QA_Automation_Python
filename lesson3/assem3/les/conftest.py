...
def make_files():
    ...
    for i in range(data["count"]):
        ...
        if checkout("cd {}; dd if=/dev/urandom of={} bs={} count=1 iflag=fullblock"
                    .format(data["folder_in"], filename, data["bs"]), ""):
...
def make_subfolder():
    ...
    if not checkout("cd {}; mkdir {}".format(data["folder_in"], subfoldername), ""):
        return '', ''
    if not checkout("cd {}/{}; dd if=/dev/urandom of={} bs=1M count=1 iflag=fullblock".format(
            data["folder_in"], subfoldername, testfilename), ""):
